 ...
 * 
 */
public class XMLFileReaderS {

	private static final Logger log = Logger.getLogger(XMLFileReaderS.class
			.getName());
	...
	public static void writeToPropertiesFiles(String filePathE,StringBuffer name) {
		...
		try {
/*			for(String filePath : propFilesSet) {
			propFilePath = 	new File(filePath);
			fileWriter = new FileWriter(propFilePath,true);
			log.info("created File at Path for Writing :"+propFilePath);
		//	BufferedWriter bWriter = new BufferedWriter(new FileWriter(propFilePath));
	//		bWriter = new BufferedWriter(fileWriter);
			}*/
			
			log.info("propMap size :"+ propMap.size());
			Map<String,NPProp> myMap = propMap;
			
			Set<String> mapKeySet =  myMap.keySet();
			for(String mapKSetEl : mapKeySet) {
				log.info("Map KeySet Elements :" + mapKSetEl);
				NPProp npProp = (NPProp)myMap.get(mapKSetEl);
				log.info("Created Properties File at:"+ mapKSetEl);
				Date calTime = Calendar.getInstance().getTime();
				int hr = calTime.getHours();
				int min = calTime.getMinutes();
				int sec = calTime.getSeconds();
		//		propFilePath = 	new File(propFilePath+"_"+hr+min+sec);
				propFilePath = 	new File(npProp.getFilePath());
				log.info("Created File at :"+ propFilePath);
				//npProp.getFilePath()
			//	fileWriter = new FileWriter(propFilePath+"_"+hr+min+sec,true);
				fileWriter = new FileWriter(propFilePath,true);
				bWriter = new BufferedWriter(fileWriter);
				log.info("Writing the Property Value"+npProp.getPropValue()+" to the property Key: "+ npProp.getPropKey());
				bWriter.append(npProp.getPropKey()+"="+npProp.getPropValue());
				bWriter.newLine();
				//bWriter.append(name);
				}
			} catch(Exception ex) {
